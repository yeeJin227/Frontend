...
export type ArtistTableColumn<T> = {
  key: keyof T | string;
  header: string;
  width?: string;
...
export type ArtistDataTableProps<T> = {
  columns: ArtistTableColumn<T>[];
  rows: T[];
...
export default function ArtistDataTable<T>({
  columns,
  rows,
  rowKey,
  selectable = true,
  ...
  const handleSort = (column: ArtistTableColumn<T>) => {
